...
<div x-data="{ modalOpen: @js($open) }">
    <div x-ref="{{ Str::camel($id) }}" id="{{ $id }}" x-cloak x-show="modalOpen"
        ...
        class="fixed inset-0 z-30 flex items-end justify-center bg-black/20 backdrop-blur-md sm:items-start sm:overflow-y-auto {{ $containerClass }}"
        ...
        <div x-show="modalOpen"
            ...
            {{ $attributes->merge(['class' => 'flex w-full max-w-xl flex-col gap-4 overflow-hidden rounded-t-2xl sm:rounded-radius bg-surface text-on-surface dark:bg-surface-dark-alt dark:text-on-surface-dark sm:my-32 overflow-y-auto sm:overflow-y-hidden max-h-[90svh] sm:max-h-[unset]']) }}>
            {{ $slot }}
        </div>
    </div>
</div>
