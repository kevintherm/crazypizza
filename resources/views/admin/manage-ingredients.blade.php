        ...
        <form x-on:submit.prevent="$store.mg.filters.process">
            <div class="p-6">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" x-on:click="$store.mg.filters.hide()"
                ...
                <div class="flex flex-col gap-2 md:gap-4">
                    <template x-for="(filter, index) in $store.mg.availableFilters" :key="filter.name">
                        <div>
                            <label :for="filter.column" class="mb-2 w-fit text-sm" x-text="filter.name"></label>

                            <template x-if="filter.type == 'number' || filter.type == 'text'">
                                <input :id="filter.column" :type="filter.type" :min="filter.min"
                                    :max="filter.max" :step="filter.step" :minlength="filter.minLength"
                                    :maxlength="filter.maxLength" x-mask:dynamic="filter.mask"
                                    x-on:input="filter.type == 'number' ? $store.when.inputNumber($event) : null"
                                    :name="filter.column"
                            ...
                            <template x-if="filter.type == 'range'">
                                <div class="flex items-center gap-2">
                                    <input type="text" x-on:input="$store.when.inputNumber"
                                        :name="`${filter.column}.min`"
                            ...
                            <template x-if="filter.type == 'select'">
                                <div x-data="combobox({ allOptions: filter.options, selectedOption: filter.selectedOption })" class="w-full"
                                    x-on:keydown="handleKeydownOnOptions($event)"
                                    x-on:keydown.esc.window="isOpen = false, openedWithKeyboard = false">
                                    <div class="relative">
                                        <button x-ref="comboboxTrigger" type="button"
                                            ...
                                            role="combobox" :aria-controls="filter.name" aria-haspopup="listbox"
                                        ...
                                        <input :id="filter.column" :name="filter.column" x-ref="hiddenTextField" hidden="">

                                        <div x-show="isOpen || openedWithKeyboard" :id="filter.name"
                                            class="z-40 flex flex-col overflow-hidden rounded-radius border border-outline bg-surface-alt dark:border-outline-dark dark:bg-surface-dark-alt"
                                            role="listbox" :aria-label="filter.name + 'list'"
                                            x-on:click.outside="isOpen = false, openedWithKeyboard = false"
                                            x-on:keydown.down.prevent="$focus.wrap().next()"
                                            x-on:keydown.up.prevent="$focus.wrap().previous()" x-transition
                                            x-trap.noscroll="openedWithKeyboard"
                                            x-anchor.bottom-start.offset.4="$refs.comboboxTrigger">
                ...
                <div class="flex justify-end gap-4">
                    <button type="button" x-on:click="$store.mg.filters.hide()"
                        class="rounded-radius border border-surface-alt bg-surface-alt px-4 py-2 text-center text-xs font-medium tracking-wide text-on-surface-strong transition hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-surface-alt active:opacity-100 active:outline-offset-0 disabled:cursor-not-allowed disabled:opacity-75 dark:border-surface-dark-alt dark:bg-surface-dark-alt dark:text-on-surface-dark-strong dark:focus-visible:outline-surface-dark-alt whitespace-nowrap">Cancel</button>
                    <button type="submit"
                        class="rounded-radius border border-primary bg-primary px-4 py-2 text-center text-sm font-medium tracking-wide text-on-primary transition hover:opacity-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary active:opacity-100 active:outline-offset-0 disabled:cursor-not-allowed disabled:opacity-75 dark:border-primary-dark dark:bg-primary-dark dark:text-on-primary-dark dark:focus-visible:outline-primary-dark whitespace-nowrap">Apply</button>
                </div>
            </div>
        </form>
    </x-modal>
