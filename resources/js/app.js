 ...
 */
export const createDataTableStore = (config) => ({
    ...
    init() {
        ...
        this.createUpdate = this.createModalHandler(this.config.selectors.createUpdate, {
            hide: () => { // Override hide to also clear selected item
    ...
    deleteItem(id, name = '', column = null) {
        ...
        this.confirm.onConfirm = () => this.handleApiCall(
            ...
            () => {
                if (column) { // Deleting a property of an item (e.g., an image)
                    const itemIndex = this.items.findIndex(i => i.id === id);
                    if(itemIndex > -1) this.items[itemIndex][column] = null;
                } else { // Deleting the whole item
    ...
    createModalHandler(selector, customMethods = {}) {
        ...
        const base = {
            element,
            show: () => $.notifier.modal(element, 'show'),
            hide: () => $.notifier.modal(element, 'hide'),
    ...
    handleApiCall(promise, successCallback, errorCallback = null) {
        ...
        promise
            .then(res => {
                ...
                $.notifier.toast({ variant: 'success', title: 'Success', message });
                successCallback(res);
            })
            .catch(err => {
                const message = err?.response?.data?.message || err.message || 'An error occurred.';
                $.notifier.toast({ variant: 'danger', title: 'Oops...', message });
